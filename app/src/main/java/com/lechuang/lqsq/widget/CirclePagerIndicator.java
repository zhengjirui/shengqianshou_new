...
//        }
        ...
        if (mCenterHorizontal) {
            xOffset += ((width - paddingLeft - paddingRight) - (viewPagerRealSize * circleAndSpace)) / 2.0f;
        }
        ...
        //绘制所有圆点
        for (int i = 0; i < viewPagerRealSize; i++) {
        ...
        float cx = (!mIsFollow ? (mFollowPage % viewPagerRealSize) : (mCurrentPage % viewPagerRealSize)) * circleAndSpace;
//        Log.d("Draw", "onDraw:------>>>>  " + mFollowPage + ":" + mCurrentPage);
        ...
        cX = xOffset + cx;
        cY = yOffset;
        if (cX <= (xOffset + (viewPagerRealSize - 1) * circleAndSpace)) {
    ...
    public void setViewPagerRealSize(int size) {
        viewPagerRealSize = size;
    }
...
//        Log.d("index", "onPageScrolled: " + position);
