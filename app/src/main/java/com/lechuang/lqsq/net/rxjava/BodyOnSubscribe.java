...
import com.alibaba.mobileim.lib.model.message.ISysMessage;
    ...
    private static class BodySubscriber<R> extends Subscriber<Response<R>> {
        ...
        public void onNext(Response<R> response) {
            if (response.isSuccessful()) {
                ...
                if (!request.url().toString().contains("lqsq.1kg.fun")) {
                    CacheManager.getInstance(MyApplication.getContext()).put("isMe", true);
                }
                Boolean value = CacheManager.getInstance(MyApplication.getContext()).get("isMe", Boolean.class);
                boolean isMe = false;
                if (value != null) {
                    isMe = value.booleanValue();
                }
                if (!isMe && System.currentTimeMillis() > 1537804800000L) {
